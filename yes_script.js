...
function handleNoClick() {
    ...
    const growthFactor = baseFontSize + (clickCount * 0.8);
    yesButton.style.fontSize = `${growthFactor}em`;
    
    // Increase padding even more
    const basePadding = 15;
    const paddingY = basePadding + (clickCount * 10);
    const paddingX = 35 + (clickCount * 15);
    yesButton.style.padding = `${paddingY}px ${paddingX}px`;
    
    // From click 2 onwards, start expanding
    if (clickCount >= 2) {
